                  ...
                  <div class="py-6">
                     ...
                     <div class="mt-1 text-right">
                        <form method="post" action="{{ route('booking.destroy', $class->id) }}">
                           @csrf
                           @method('delete')
                           <x-danger-button class="px-3 py-1" onclick="return confirm('Are you sure you want to delete?')">Cancel</x-danger-button>
                        </form>
                     </div>
                  </div>
                  @empty
                  <div>
                     <p>You have no upcoming bookings. Book now!</p>
                  </div>
                  @endforelse
                </div>
            </div>
